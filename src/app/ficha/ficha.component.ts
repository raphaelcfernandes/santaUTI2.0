...
export class FichaComponent implements OnInit,OnDestroy {
        ...
        if(this.fichaObject.Respiratorio.viasAereas === 'Natural') {
            ...
            if(this.fichaObject.Respiratorio.UsoOxigenio === 'Não'){
                ...
                Object.keys(this.fichaObject.Respiratorio.UsoOxigenio).forEach(item => {
                    ...
                    if(item === 'Sob Cateter Nasal' || item === 'Em Máscara de Reinalação') {
                        this.respiratorioString += item.toLowerCase() + ' com fluxo de '
                                                + this.fichaObject.Respiratorio.UsoOxigenio[item] + ' L/min.';
                    }
                });
            }
        } else{
            Object.keys(this.fichaObject.Respiratorio.ViasAereas).forEach(item =>{
                ...
                if(item === 'Traqueostomia') {
                    this.respiratorioString = 'Paciente traqueostomizado (pressão do CUFF ' + this.fichaObject.Respiratorio.ViasAereas.pressaoCuff+').';	
                }
            })
        }
    }
    ...
    private raioxTorax(): void {
        if(this.fichaObject.Exames.raioxTorax === 'Não realizou/sem resultados'||
            ...
            this.respiratorioString+= 'Raio-x de tórax não realizado ou sem resultados.';
    ...
    private hemodinamicamente() {
        this.hemodinamicoString = 'Paciente hemodinamicamente ' + this.fichaObject.FolhasBalanco.hemodinamicamente.toLowerCase()
                                + ' compensado em ritmo ' + this.fichaObject.MonitorMultiparametrico.ritmo.toLowerCase()
                                + ', FC ' + this.fichaObject.MonitorMultiparametrico.frequenciaCardiaca ;
    }
    ...
    private itensMonitorMultiparametrico(){
        ...
        + this.fichaObject.Hemodinamico.extremidadesColoracao.toLowerCase() + ' e ' + this.fichaObject.Hemodinamico.extremidadesTemperatura.toLowerCase() 
        + ', perfusão capilar ' + this.fichaObject.Hemodinamico.perfusaoCapilar.toLowerCase()  
